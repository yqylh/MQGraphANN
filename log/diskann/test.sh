dataset=(fashionmnist glove lastfm mnist nytimes sift)
...
for i in ${dataset[@]}; do
    ...
    # {
    #     $diskann/utils/compute_groundtruth --data_type float --dist_fn l2 --base_file ./$i/base.fbin --query_file ./$i/query.fbin --gt_file ./$i/gt100 --K 100
    # }
    # 构建内存索引 finished
    # {
    #     # R 也就是 M 邻居数
    ...
    #         $diskann/build_memory_index  --data_type float --dist_fn l2 --data_path ./$i/base.fbin --index_path_prefix $indexName -R $R -L $L --alpha $alpha --num_threads $num_threads
    #     done
    # }
    # 查询
    {
        ...
        for R in ${R_all[@]}; do
            echo "====== dataset: $i, R: $R, L: $L, alpha: $alpha, num_threads: $num_threads ======"
            indexName="${i}/index_R${R}_L${L}_A${alpha}"
            $diskann/search_memory_index  --data_type float --dist_fn l2 --index_path_prefix $indexName --query_file ./$i/query.fbin --gt_file ./$i/gt100 -K 100 -L $L --result_path ./$i/
        done
    }
done