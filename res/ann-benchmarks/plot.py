...
def create_plot(all_data, raw, x_scale, y_scale, xn, yn, fn_out, linestyles, batch):
    ...
    ax = plt.gca()
    ax.set_ylabel(ym["description"],fontsize=26)
    ax.set_xlabel(xm["description"],fontsize=26)
    ...
    ax.set_title(get_plot_label(xm, ym), size=26)
    plt.gca().get_position()
    # plt.gca().set_position([box.x0, box.y0, box.width * 0.8, box.height])
    ax.legend(handles, labels, loc="center right", bbox_to_anchor=(1, 0.85), prop={"size": 22})
    ...
    if "lim" in ym:
        plt.ylim(ym["lim"])
    ax.tick_params(axis='x', which='major', labelsize=22)  # 修改 x 轴主要刻度标签大小
    ax.tick_params(axis='x', which='minor', labelsize=22)  # 修改 x 轴次要刻度标签大小
    ax.tick_params(axis='y', which='major', labelsize=22)  # 修改 y 轴主要刻度标签大小
    ax.tick_params(axis='y', which='minor', labelsize=22)  # 修改 y 轴次要刻度标签大小
...
if __name__ == "__main__":
    ...
    import re
    pattern = re.compile(r'Recall=(\d+\.\d+)\s+avgTime=(\d+\.\d+)us')
    hnsw = []
    with open("../../result/hnsw1.txt", 'r') as file:
        for line in file:
            match = pattern.search(line)
            if match:
                recall = float(match.group(1))
                avg_time = float(match.group(2))  # Convert microseconds to milliseconds
                hnsw.append(('hnsw', recall, 1000000/avg_time))
    nsw = []
    with open("../../result/nsw1.txt", 'r') as file:
        for line in file:
            match = pattern.search(line)
            if match:
                recall = float(match.group(1))
                avg_time = float(match.group(2))  # Convert microseconds to milliseconds
                nsw.append(('nsw', recall, 1000000/avg_time))
    mq = []
    with open("../../result/mq1.txt", 'r') as file:
        for line in file:
            match = pattern.search(line)
            if match:
                recall = float(match.group(1))
                avg_time = float(match.group(2))  # Convert microseconds to milliseconds
                mq.append(('mq', recall, 1000000/avg_time))
    data_points = {'hnsw': hnsw, 'nsw': nsw, 'mq': mq}
    create_plot(
        data_points, args.raw, args.x_scale, args.y_scale, args.x_axis, args.y_axis, args.output, linestyles, args.batch
    )


# 10 11 12 13   x=logit y=linear
# 1 7 9 x=logit y=linear
# 8 x=a2 y=linear