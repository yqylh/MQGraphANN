...
def create_plot(all_data, raw, x_scale, y_scale, xn, yn, fn_out, linestyles, batch):
    ...
    ax.legend(handles, labels, loc="center right", bbox_to_anchor=(1.22, 0.6), prop={"size": 22})
...
if __name__ == "__main__":
    ...
    for dsname in dataset:
        args.output = "results/mq_cluster/mq_%s.pdf" % dsname
        print("writing output to %s" % args.output)
        data_points = {}
        for num in cluster_num:
            data = []
            with open("../../result/mq_cluster/"+dsname+"_"+num+".txt", 'r') as file:
                for line in file:
                    match = pattern.search(line)
                    if match:
                        recall = float(match.group(1))
                        avg_time = float(match.group(2))  # Convert microseconds to milliseconds
                        data.append((num, recall, 1000000/avg_time))
            data_points[num] = data
        create_plot(
